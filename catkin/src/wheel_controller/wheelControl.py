...
PPR = 6*4*30.0 # Encoder pulses per revolution
MAX_RPM = 11000/30.0 # Motor max rpm (after gearbox)

filterFs = 110.0 # Sample rate at which to run the motor speed filter (Hz)

FREQUENCY = 10.0 # Frequency to run the main PWM loop (Hz)
...
def filterCallback(event):
    global filterB, filterZ, filteredSpeed, motorSpeed
...
def encACallback(channel):
    ...
    if GPIO.input(encA) == GPIO.input(encB):
        motorPosition += 1/PPR
        motorSpeed = 1/(now - lastTime)/PPR/60
...
def main():
    ...
    filterB = signal.firwin(256, FREQUENCY, fs=filterFs)
    ...
    while not rospy.is_shutdown():
        ...
        rospy.loginfo("Position: " + str(motorPosition) + " Speed: " + str(filteredSpeed))
